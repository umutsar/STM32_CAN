...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if(
			RxNextionEvent[0] == 255 &&
			RxNextionEvent[1] == 255
	  )
	{
	...
	// Farı aç
	if (strcmp((const char*)RxNextionEvent, "lediyakFFF") == 0)
	{
	...
	//Farı kapat
	if (strcmp((const char*)RxNextionEvent, "ledisondur") == 0)
	{
	...
	if(strcmp((const char*)RxNextionEvent, "akishiziFF") == 0)
	{
		if(sw2 == 1) {
			sw2 = 0;
	...
	if(strcmp((const char*)RxNextionEvent, "bataryaFF") == 0)
	{
		if(sw1 == 1) {
			sw1 = 0;
		}
	...
	if(strcmp((const char*)RxNextionEvent, "loraverisi") == 0)
	{
		if(sw3 == 1) {
			sw3 = 0;
	...
	if(strcmp((const char*)RxNextionEvent, "gostergeFF") == 0)
	{
		if(sw0 == 1) {
			sw0 = 0;
	...
	HAL_UART_Receive_IT(&huart1, RxNextionEvent, 10);
}
